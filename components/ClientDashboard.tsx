  ...
  const handleApprove = async () => {
    try {
      ...
      await writeContractAsync({ 
        ...
        args: [CONTRACT_ADDRESS, parseUnits(deposit, 6)] 
      });
  ...
  const handleCreateStream = async () => {
    try {
      ...
      await writeContractAsync({ 
        ...
        args: [provider as `0x`, parseUnits(rate, 6), parseUnits(deposit, 6)] 
      });
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl h-fit">
        <h2 className="text-xl font-bold text-blue-400 mb-6 flex items-center gap-2">
          <Zap className="w-5 h-5" /> Start New Stream
        </h2>
        
        <div className="space-y-4">
            <div>
                <label className="text-xs text-slate-500 font-bold block mb-1">PROVIDER ADDRESS</label>
                <input type="text" value={provider} onChange={(e) => setProvider(e.target.value)} className="w-full bg-slate-950 border border-slate-700 p-3 rounded-lg text-sm text-white focus:outline-none focus:border-blue-500 transition" />
            </div>
            <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs text-slate-500 font-bold block mb-1">RATE (USDC/S)</label>
                  <input type="number" value={rate} onChange={(e) => setRate(e.target.value)} className="w-full bg-slate-950 border border-slate-700 p-3 rounded-lg text-sm text-white focus:outline-none focus:border-blue-500 transition" />
                </div>
                <div>
                  <label className="text-xs text-slate-500 font-bold block mb-1">DEPOSIT (USDC)</label>
                  <input type="number" value={deposit} onChange={(e) => setDeposit(e.target.value)} className="w-full bg-slate-950 border border-slate-700 p-3 rounded-lg text-sm text-white focus:outline-none focus:border-blue-500 transition" />
                </div>
            </div>
            ...
            <div className="flex gap-3 pt-2">
                <button onClick={handleApprove} className="flex-1 bg-slate-800 hover:bg-slate-700 text-white py-3 rounded-lg font-bold transition">1. Approve</button>
                <button onClick={handleCreateStream} className="flex-1 bg-blue-600 hover:bg-blue-500 text-white py-3 rounded-lg font-bold transition">2. Start</button>
            </div>
        </div>
                    ...
                    {paginatedStreams.map((s) => {
                        ...
                        <div key={s.id.toString()} onClick={() => setStreamIdToMonitor(s.id.toString())}
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                {s.isActive ? (
                                    isRunOut ? <span className="text-orange-400 font-bold">● Depleted</span> : <span className="text-emerald-400 font-bold">● Running</span>
                                ) : (
                            ...
                            {!s.isActive && (
                                <div className="flex justify-between text-[10px] bg-slate-900/50 p-1 rounded mt-1">
                                    ...
                                    <span className="text-emerald-300">Ret: {refunded.toFixed(4)} $</span>
                                </div>
                            )}
                        </div>
                    )})}
                ...
                {totalPages > 1 && (
                  <div className="flex justify-between items-center mt-4 pt-2 border-t border-slate-800/50">
                    <button 
                      ...
                      className="p-2 rounded hover:bg-slate-800 disabled:opacity-30 disabled:hover:bg-transparent transition"
                    >
      ...
      <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl h-fit">
        <h2 className="text-xl font-bold text-emerald-400 mb-6 flex items-center gap-2">
            <Timer className="w-5 h-5" /> Live Monitor
        </h2>
        
        <div className="flex justify-between items-end mb-6">
             <div>
                 <label className="text-xs text-slate-500 font-bold block">STREAM ID</label>
                 <div className="text-2xl font-mono text-white font-bold">{streamIdToMonitor || "--"}</div>
             </div>
             <button onClick={() => { refetchStream(); refetchList(); }} className="p-2 bg-slate-800 rounded hover:bg-slate-700 transition">
        ...
        <div className="bg-black/40 rounded-xl p-6 flex flex-col items-center justify-center relative overflow-hidden border border-slate-800 min-h-[250px]">
           ...
           <div className="w-full space-y-4">
             <div className="flex justify-between text-sm">
                 ...
                 <span className="text-emerald-400 font-mono font-bold">{currentCost.toFixed(4)} USDC</span>
             </div>
             <div className="relative w-full h-3 bg-slate-800 rounded-full overflow-hidden">
                 <div 
                    className={`absolute top-0 left-0 h-full transition-all duration-1000 ${progressPercent > 90 ? 'bg-red-500' : 'bg-blue-500'}`} 
           ...
           {isDepleted && (
                <div className="absolute inset-0 bg-red-900/40 flex items-center justify-center backdrop-blur-sm">
                   <div className="bg-red-900 text-white px-4 py-2 rounded-full font-bold flex items-center gap-2 animate-pulse shadow-lg">
        ...
        <button 
            ...
            disabled={!streamIdToMonitor} 
            className="w-full mt-6 bg-red-900/50 hover:bg-red-600 border border-red-800 text-red-100 py-4 rounded-xl font-bold transition disabled:opacity-50 disabled:cursor-not-allowed"
        >
            Stop & Settle
        </button>
      </div>
